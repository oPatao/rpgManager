...
export const AssetModal = () => {
  ...
  const togglePreview = () => {
    if (isPlayingPreview) {
      ...
      const urlVal = streamUrl.trim();
      if (!urlVal) return;
      stopPreview();
      const audio = new Audio(urlVal);
      previewAudioRef.current = audio;
      audio.play().then(() => {
        setIsPlayingPreview(true);
      }).catch(err => {
        console.error("Erro ao reproduzir preview de áudio:", err);
        alert("Não foi possível reproduzir este áudio. Verifique se o link é direto (ex: .mp3, .ogg, .wav) e se o servidor permite reprodução web.");
        setIsPlayingPreview(false);
      });
  ...
  useEffect(() => {
    ...
    if (modalState.isOpen && modalState.type === 'track') {
      const isStream = modalState.data?.isStream || (typeof modalState.data?.fileData === 'string' && modalState.data.fileData.startsWith('http'));
      if (isStream) {
        setTrackSourceType('stream');
        setStreamUrl(modalState.data?.fileData || '');
      } else {
  ...
  const saveAsset = async (e) => {
    ...
    try {
      ...
      if (modalState.type === 'track' && trackSourceType === 'stream') {
        const streamVal = streamUrl.trim();
        if (!streamVal) {
          alert("Por favor, informe a URL do áudio ou stream web.");
          setIsLoadingLocal(false);
          return;
        }
        data.fileData = streamVal;
        data.isStream = true;
         ...
         if (modalState.type === 'npc') {
             ...
             if (modalState.type === 'track') data.isStream = false;
         }
      } else if (isEditing) {
         data.fileData = modalState.data.fileData;
         data.isStream = modalState.data.isStream || false;
         if (modalState.type === 'npc') {
             data.variants = keptVariants;
             if (data.variants.length > 0) data.fileData = data.variants[0];
         }
      } else if (modalState.type === 'track') {
         alert("Por favor, selecione um arquivo de áudio ou informe um link de stream.");
         setIsLoadingLocal(false);
         return;
      }
        ...
        <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl w-full max-w-md shadow-2xl animate-fade-in-up custom-scrollbar max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={saveAsset} className="flex flex-col gap-4">
            ...
            {modalState.type === 'track' && (
              <>
                {/* SELETOR DE ORIGEM DO ÁUDIO (ARQUIVO LOCAL VS LINK/STREAM) */}
                <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 gap-1 mb-1">
                  <button
                    type="button"
                    onClick={() => { setTrackSourceType('file'); stopPreview(); }}
                  ...
                  >
                    <Music className="w-3.5 h-3.5" /> Arquivo Local (MP3/WAV)
                  </button>
                  <button
                    type="button"
                    onClick={() => setTrackSourceType('stream')}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
                      trackSourceType === 'stream'
                        ? 'bg-purple-600 text-white shadow-sm'
                        : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    <Radio className="w-3.5 h-3.5" /> Link / Web Stream (URL)
                  </button>
                </div>

                {trackSourceType === 'file' ? (
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 uppercase font-bold">Upload de Áudio (MP3/WAV/OGG)</label>
                    <input 
                      type="file" 
                      name="fileInput" 
                      accept="audio/*" 
                      required={!isEditing || modalState.data?.isStream} 
                    ...
                    />
                    {isEditing && !modalState.data?.isStream && (
                      <span className="text-[10px] text-emerald-500 italic mt-1">
                        Áudio já guardado. Envie um novo arquivo apenas se desejar substituir.
                      </span>
                    )}
                  </div>
                ) : (
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 uppercase font-bold flex items-center justify-between">
                      <span>URL Direta de Áudio ou Web Stream</span>
                      <span className="text-[10px] text-purple-400 font-mono font-normal">HTTP / HTTPS</span>
                    </label>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-slate-950 border border-slate-700 rounded-lg p-3 text-white text-sm focus:border-purple-500"
                        placeholder="https://exemplo.com/musica.mp3 ou URL de rádio web..."
                        required={trackSourceType === 'stream'}
                      />
                      <button
                        type="button"
                        onClick={togglePreview}
                        disabled={!streamUrl.trim()}
                        className="px-3 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 text-slate-200 border border-slate-700 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors shrink-0"
                        title="Testar reprodução deste link de áudio"
                      >
                        {isPlayingPreview ? (
                          <>
                            <Square className="w-3.5 h-3.5 text-amber-400 fill-amber-400" /> Parar
                          </>
                        ) : (
                          <>
                            <Play className="w-3.5 h-3.5 text-emerald-400 fill-emerald-400" /> Testar
                          </>
                        )}
                      </button>
                    </div>
                    <span className="text-[10px] text-slate-400 italic mt-1">
                      Suporta links diretos de áudio (.mp3, .ogg, .wav) e transmissões contínuas de rádios web/stream. Economiza espaço local e mantém backups ultra leves!
                    </span>
                  </div>
                )}
