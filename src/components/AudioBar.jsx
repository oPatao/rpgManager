...
import { Music, Play, Pause, SkipForward, Repeat, Volume2, FastForward, CloudRain, Radio, Zap, RefreshCw, Clock, X } from 'lucide-react';
...
export const AudioBar = () => {
  ...
  const isLiveStream = !isFinite(audioProgress.duration) || currentTrack?.isStream || (typeof currentTrack?.fileData === 'string' && currentTrack.fileData.startsWith('http'));

  // Seek on click
  const handleSeek = (e) => {
    if (isLiveStream || !audioProgress.duration) return;
  ...
  const currentPercent = isLiveStream 
    ? 100 
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 h-16 bg-slate-900/95 backdrop-blur-md border-t border-slate-800/90 px-4 py-2 text-slate-300 shadow-2xl flex items-center justify-between gap-3 sm:gap-6">
      ...
      <div className="flex items-center gap-3 min-w-0 max-w-[220px] sm:max-w-[280px]">
        <div className="w-10 h-10 rounded-lg bg-slate-800 border border-slate-700/60 overflow-hidden shrink-0 flex items-center justify-center relative group">
          {currentTrack?.fileData && !isLiveStream ? (
            <img src={getAssetUrl(currentTrack.fileData)} alt="" className="w-full h-full object-cover" />
          ) : isLiveStream ? (
            <Radio className={`w-5 h-5 ${isPlaying ? 'text-emerald-400 animate-pulse' : 'text-slate-500'}`} />
          ) : (
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs font-semibold text-slate-100 truncate flex items-center gap-1.5">
            <span className="truncate">{currentTrack ? currentTrack.name : 'Nenhuma Trilha Tocando'}</span>
            {isLiveStream && (
              <span className="text-[9px] px-1 py-0.2 bg-emerald-950 text-emerald-300 border border-emerald-800 rounded font-bold shrink-0">
                LIVE
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-0.5">
            {isLiveStream ? (
              <span className="text-[10px] text-emerald-400 font-mono font-bold flex items-center gap-1">
                ...
                AO VIVO ({formatTime(audioProgress.time)})
              </span>
            ) : (
      ...
      <div className="hidden lg:flex items-center gap-2 max-w-[240px] border-l border-slate-800 pl-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider flex items-center justify-between">
            <span>A Seguir</span>
            {queuedTrack && <span className="text-[9px] text-amber-400 font-mono">ao fim</span>}
          </div>
          {queuedTrack ? (
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs text-amber-300 truncate font-medium flex-1" title={queuedTrack.name}>
                {queuedTrack.name}
              </span>
              <button
                type="button"
                onClick={() => {
                  publishScene({
                    ...activeScene,
                    audio: { ...currentAudio, trackId: queuedTrack.id, transition: 'fade', loop: true, playTimestamp: Date.now() }
                  });
                  setQueuedTrackId(null);
                }}
                className="px-1.5 py-0.5 bg-purple-950/80 hover:bg-purple-900 border border-purple-700/60 text-purple-300 rounded text-[9px] font-bold transition-colors"
                title="Transicionar imediatamente com Fade"
              >
                Fade
              </button>
              <button
                ...
                onClick={() => {
                  publishScene({
                    ...activeScene,
                    audio: { ...currentAudio, trackId: queuedTrack.id, transition: 'instant', loop: true, playTimestamp: Date.now() }
                  });
                  setQueuedTrackId(null);
                }}
                className="px-1.5 py-0.5 bg-amber-950/80 hover:bg-amber-900 border border-amber-700/60 text-amber-300 rounded text-[9px] font-bold transition-colors"
                title="Mudar imediatamente sem fade (Corte Seco)"
              >
                Instant
              </button>
              <button
